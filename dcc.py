...
def dcc_main(
    ...
    dex2c_file_data = dex2c_file_data.replace(
        'env->FindClass("np/dex2c/Leeapk");',
        'env->FindClass("' + custom_loader.replace(".", "/") + '");',
    )
    dex2c_file_data = dex2c_file_data.replace(
        "Java_np_dex2c_Leeapk", "Java_" + custom_loader.replace(".", "_")
    )
    ...
    if is_apk(apkfile) and outapk:
        ...
        loader_file_path = "loader/Leeapk.smali"
        ...
        filedata = filedata.replace(
            "Lnp/dex2c/Leeapk;", "L" + custom_loader.replace(".", "/") + ";"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--custom-loader",
        default="np.dex2c.Leeapk",
        help="Loader class, default: np.dex2c.Leeapk",
    )
    parser.add_argument(
        "--skip-synthetic",
        action="store_true",
